...
export async function logout(){if(configured()){const client=await db();await client.auth.signOut();}redirect('/login');}
export async function mutate(op:Operation,input:unknown):Promise<Result>{
 const {client,manager}=await session();
...
export async function receive(input:unknown):Promise<Result>{
 const {client,manager}=await session();if(!manager)return {ok:false,message:'Permisos insuficientes'};
